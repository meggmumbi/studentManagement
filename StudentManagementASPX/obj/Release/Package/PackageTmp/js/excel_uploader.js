...
//                console.log("Pushing batch " + batch + " to server | start = " + start + " stop = " + stop);
	 ...
	 */
	// alert(username);
	 //var victor =username;//"Today";
	// alert(victor);
        let payload = $.extend({column_map: JSON.parse(serverColumnNames), data: data,userUserName:username,userPassword: password}, extraData);
		//alert(extraData);
	//dataType
	//console.log(payload);
	//alert(data[1].length);
        ...
        //alert();
        $.ajax({
            url: "http://localhost:8072/api/Values/" + url,
            type: "POST",
            crossDomain: true,
